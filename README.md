...
- **`PracticeX.Agent.Ui` is Windows-only at runtime.** It targets WPF on `net9.0-windows`. The repository now builds on macOS/Linux by enabling Windows targeting packs during `dotnet build PracticeX.sln`, but only the API, CLI, discovery services, tests, and React command-center app are expected to run locally on macOS.
- **Executable projects roll forward to newer runtimes.** The repo targets `.NET 9`, but `Directory.Build.props` sets `RollForward=Major` so local `dotnet run` / `dotnet test` can still execute on machines that currently only have the `.NET 10` shared runtime installed.
